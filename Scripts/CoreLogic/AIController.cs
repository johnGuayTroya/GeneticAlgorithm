...
public class AIController : MonoBehaviour
{
    ...
    private LogicState _currentLogicState;
    ...
    private void ExpectMiniMax()
    {
        ...
        Debug.Log($"---> Action:{_attackToDo}:{value}");
    }
    ...
    private float MaxValor(LogicState state)
    {
        ...
        _alpha = -20;
        var children = state.GenerateChildren(0, _player.Attacks);
        ...
        foreach (var (att,logicState) in children)
        {
            ...
            if (val > _alpha)
            {
                ...
                _attackToDo.Target = GameState.ListOfPlayers.Players[1];
            }
        }
        return _alpha;
    }
    ...
    private float MinValor(LogicState state)
    {
        ...
        _beta = +20;
        var children = state.GenerateChildren(1, GameState.ListOfPlayers.Players[1].Attacks);
    ...
    {
        if (state.HitPoints[0] <= 0) return -20;
        if (state.HitPoints[1] <= 0) return 20;
        return state.HitPoints[0] - state.HitPoints[1];
    }
